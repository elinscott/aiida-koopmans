...
class TestSinglepointDFPTAutoEps:
    ...
    def test_eps_kpoints_changes_only_the_mesh_not_the_namelist(
        self, ph_codes, silicon_structure, kmesh, denser_kmesh
    ):
        """``eps_kpoints`` swaps the mesh alone; the chain's own cutoffs still apply.

        A mutant that builds the dielectric overrides from scratch instead
        of copying the chain's own ``overrides["scf"]`` would drop the
        distinctive ``ecutwfc`` here.
        """
        wg = SinglepointDFPTWorkflow.build(
            codes=ph_codes,
            structure=silicon_structure,
            ...
            kpoints=kmesh,
            eps_kpoints=denser_kmesh,
            overrides={"scf": {"pw": {"parameters": {"SYSTEM": {"ecutwfc": 55.0}}}}},
            pseudo_family="SSSP/1.3/PBE/efficiency",
            eps_inf="auto",
        )
        dielectric = wg.tasks["dielectric"]
        assert dielectric.inputs["scf_kpoints"].value.uuid == denser_kmesh.uuid
        dielectric_scf_overrides = dielectric.inputs["overrides"].value["scf"]
