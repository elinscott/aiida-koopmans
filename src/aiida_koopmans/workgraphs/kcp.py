...
def KoopmansDSCFWorkflow(
    ...
    * ``'mlwfs'`` / ``'projwfs'`` (periodic): wannierise → fold to the
    ...
    merge_groups = None
    # The initialization's pw.x nscf and kcp.x dft_init scalar outputs,
    # which the interpolated band structure reads to align kcp.x's
    # absolute energy scale to pw.x's; only the Wannier route runs both.
    nscf_output_parameters = None
    dft_init_output_parameters = None
    ...
    if wannier_init:
        ...
        nscf_output_parameters = init["nscf_output_parameters"]
        dft_init_output_parameters = init["dft_init_output_parameters"]
...
def _interpolate_bands(
    *,
    ...
    ``dos`` only when ``do_dos``. ``nscf_output_parameters`` and
    ``dft_init_output_parameters`` put the returned bands on pw.x's
    absolute energy scale; see :func:`DscfBandStructureTask`.
    """
