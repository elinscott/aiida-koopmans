    ...
    *,
    ...
    for orbital in orbitals:
        ...
        namelist = {
            ...
            **owned("kcw.SCREEN", {"i_orb": int(orbital["index"]), "check_spread": False}),
        }
...
class KcwOverrides(TypedDict, total=False):
    """User kcw.x namelist keywords, merged into :func:`RunDFPT`'s namelists.
...
def RunDFPT(
    ...
    if alpha_guess is not None:
        ...
        screen_namelist.update(owned("kcw.SCREEN", {"check_spread": bool(check_spread)}))
        screen_inputs: dict[str, Any] = {
            "code": kcw_code,
...
def SinglepointDFPTWorkflow(
    ...
    (namelist keywords included) and the same mesh, unless ``eps_kpoints``
    gives that ground state a mesh of its own — eps_inf converges much more
    slowly with k-points than the singlepoint itself, so a denser grid
    there does not mean the chain's scf needs to match it.
