...
def display_name_for(orbital: VariationalOrbital) -> str:
    ...
    ``Orbital 5``, or ``Orbital 5 (spin up)`` where the run resolves the
    two channels separately. The band index is the orbital's own 1-based
    per-spin index, not its position in any fan-out.
    """
    spin = SpinChannel(orbital["spin"])
    if spin is SpinChannel.NONE:
